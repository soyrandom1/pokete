...
# Mock scrap_engine before importing battle_animations
mock_se = MagicMock()
...
# Mock pokete base modules
mock_color_module = MagicMock()
mock_color_module.Color = type('Color', (), {
    'white': '\033[1;37m',
    'red': '\033[38;5;196m',
    'green': '\033[38;5;70m',
...
mock_release_module = MagicMock()
mock_release_module.SPEED_OF_TIME = 0.001

# Mock the settings function
mock_setting_obj = MagicMock()
mock_setting_obj.val = True
mock_settings_func = MagicMock(return_value=mock_setting_obj)
...
sys.modules['pokete.base.color'] = mock_color_module
sys.modules['pokete.release'] = mock_release_module
sys.modules['pokete.classes'] = MagicMock()
sys.modules['pokete.classes.settings'] = mock_settings_module
sys.modules['pokete.classes.fight'] = MagicMock()

# Directly import the module file
...
class TestTypeEffects(unittest.TestCase):
    ...
    def test_all_chars_are_ascii(self):
        ...
        for type_name, effects in TYPE_EFFECT_CONFIG.items():
            self.assertTrue(effects.particle_char.isascii(),
                f"{type_name} particle_char is not ASCII: {effects.particle_char}")
            for char in effects.impact_chars:
                self.assertTrue(char.isascii(),
                    f"{type_name} impact_char is not ASCII: {char}")
            self.assertTrue(effects.trail_char.isascii(),
                f"{type_name} trail_char is not ASCII: {effects.trail_char}")
...
class TestStatusEffectIndicator(unittest.TestCase):
    ...
    def test_effect_symbols_are_ascii(self):
        ...
        for effect, symbol in StatusEffectIndicator.EFFECT_SYMBOLS.items():
            self.assertTrue(symbol.isascii(),
                f"{effect} symbol is not ASCII: {symbol}")
