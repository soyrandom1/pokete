...
# Type-specific visual effects configuration (ASCII-compatible)
TYPE_EFFECT_CONFIG: dict[str, TypeEffects] = {
    "normal": TypeEffects(Color.white, "*", ["*", "x", "*"], "."),
    "fire": TypeEffects(Color.red + Color.thicc, "~", ["*", "#", "+", "*"], "o"),
    ...
    "stone": TypeEffects(Color.grey, "o", ["#", "O", "#"], "."),
    "ice": TypeEffects(Color.cyan, "*", ["+", "x", "+"], "*"),
    "flying": TypeEffects(Color.lightblue, "~", ["~", "-", "~"], "."),
    "poison": TypeEffects(Color.purple, "x", ["X", "x", "X"], "."),
    "undead": TypeEffects(Color.purple + Color.thicc, "x", ["+", "X", "+"], "."),
}
...
class DamageNumber:
    ...
    def play(self, duration: float = 0.8):
        ...
        if not are_battle_animations_enabled():
            return
